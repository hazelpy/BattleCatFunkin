...
class PlayState extends MusicBeatState
{
	...
	public static var songStages:Map<String, String> = [
		"tutorial" => "stage",
		"bopeebo" => "stage",
		"fresh" => "stage",
		"dadbattle" => "night",
	...
	override public function create()
	{
		...
		// stage loading
		if (songStages.exists(SONG.song.toLowerCase())) {
			curStage = songStages.get(SONG.song.toLowerCase());
		} else {
			curStage = "stage";
		}
		...
		{
			if (FlxG.save.data.lockedSongs != null) {
				if (FlxG.save.data.lockedSongs.contains( curSong.toLowerCase() )) {
					songLockedCondition();
				} else {
					switch (curSong.toLowerCase())
					{
	...
	function loadStage(stage:String = "stage") {
		switch(stage) {
			case 'spooky':
				halloweenLevel = true;
			...
			case 'night':
				...
				var bg:FlxSprite = new FlxSprite(-200, 100).loadGraphic(Paths.image('stages/night/sky'));
				...
				var stageFront:FlxSprite = new FlxSprite(-500, -225).loadGraphic(Paths.image('stages/night/ground'));
	...
	function songLockedCondition(time:Float = 2) {
		var dimBG = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
			dimBG.alpha = 0;
			dimBG.cameras = [camHUD];
		add(dimBG);

		var niceTry = new FlxText(160, 96, 960, "Nice try!\nThis song is locked.\nUnlock this song through its week.", 56);
			niceTry.setFormat(Paths.font('taxicab.ttf'), 56, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			niceTry.alpha = 0;
			niceTry.cameras = [camHUD];
		add(niceTry);

		FlxTween.cancelTweensOf(dimBG); // Cancel tweens of dimming background
		FlxTween.cancelTweensOf(niceTry); // Cancel tweens of alert text
	...
	override public function update(elapsed:Float)
	{
		...
		// SONG.speed modifies SCROLL SPEED.
		// MAKE SURE TO CHECK WHAT IT'S SET AS BY DEFAULT WHEN YOU FUCKING MODIFY IT !!!
		// THE GOAL: ADD SPEED UP NOTES THAT INCREASE SCROLL SPEED BY A CONSTANT AMOUNT
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				unlockFreeplayWeek(storyWeek);
				FlxG.switchState(new StoryMapState());

				// if ()
				StoryMapState.weekUnlocked[Std.int(Math.min(storyWeek + 1, StoryMapState.weekUnlocked.length - 1))] = true;
				...
				FlxG.save.data.weekUnlocked = StoryMapState.weekUnlocked;

				StoryMapState.weekCompleted[storyWeek] = true;

				FlxG.save.data.weekCompleted = StoryMapState.weekCompleted;
				FlxG.save.flush();
			}
			else
			{
	...
	function unlockFreeplayWeek(week:Int) {
		// UNLOCK WEEK OF FREEPLAY
		// sys.io.File.saveContent('my_folder/my_file.json',content); <--- TO SAVE DATA USE THIS FUNCTION

		// LOAD DATA
		var initSonglist = CoolUtil.coolTextFile(Paths.txt('freeplaySonglist'));
		var updateSonglist:Array<String> = [];
		...
		for (i in 0...initSonglist.length)
		{
			...
			if (songWeek == week) {
				...
				var tempData = "";
					...
					tempData += songChar;
					tempData += ":";
					tempData += songLockData;
				
				trace(tempData);
				updateSonglist.push(tempData);
			} else {
				updateSonglist.push(songData);
			}
		}

		sys.io.File.saveContent(Paths.txt('freeplaySonglist'), updateSonglist.join("\n"));
		trace("UNLOCKED WEEK " + week + " IN FREE PLAY");
	}
